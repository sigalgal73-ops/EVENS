...
(function () {
  ...
  `;
    ...
    .ev-action-btn {
      ...
      display: flex; align-items: center; justify-content: center;
      font-size: 22px; box-shadow: 0 3px 16px rgba(0,0,0,0.22);
      transition: transform .2s, box-shadow .2s;
      text-decoration: none;
    }
    .ev-action-btn:hover { transform: scale(1.1); box-shadow: 0 5px 20px rgba(0,0,0,0.3); }
    .ev-wa-btn  { background: #25D366; color: #fff; }
    .ev-tel-btn { background: #3d5c2e; color: #fff; }
    .ev-a11y-btn { background: #fff; color: #3d5c2e; border: 2px solid #5a7c4a; font-size: 26px; }

    /* ── פאנל נגישות ── */
    .ev-a11y-panel {
      position: fixed; left: 86px; bottom: 104px; z-index: 9997;
  ...
  a11yBar.className = 'ev-a11y-bar';

  // WhatsApp button
  const waBtn = document.createElement('a');
  waBtn.className = 'ev-action-btn ev-wa-btn';
  waBtn.href = 'https://wa.me/972524763530';
  waBtn.target = '_blank';
  waBtn.title = 'WhatsApp – מיכל';
  waBtn.innerHTML = '💬';

  // Phone button
  const telBtn = document.createElement('a');
  telBtn.className = 'ev-action-btn ev-tel-btn';
  telBtn.href = 'tel:+972524763530';
  telBtn.title = 'חייגי למיכל';
  telBtn.innerHTML = '📞';

  // Accessibility button
  const a11yBtn = document.createElement('button');
  a11yBtn.className = 'ev-action-btn ev-a11y-btn';
  ...
  a11yBar.appendChild(waBtn);
  a11yBar.appendChild(telBtn);
  a11yBar.appendChild(a11yBtn);
  document.body.appendChild(a11yBar);
  document.body.appendChild(a11yPanel);

  // Toggle panel
  a11yBtn.onclick = () => a11yPanel.classList.toggle('open');
  ...
  document.getElementById('ev-font-up').onclick = () => {
    ...
    document.querySelectorAll('.ev-bubble, .ev-opt, .ev-multi-opt, .ev-input, .ev-send').forEach(el => {
  ...
  contrastToggle.onclick = () => {
    ...
    win.style.filter = on ? 'contrast(1.5) brightness(0.95)' : '';
  };
  ...
  linksToggle.onclick = () => {
    ...
    document.querySelectorAll('.ev-bubble a').forEach(a => {
      ...
      a.style.border = on ? '1px solid #3d5c2e' : '';
      a.style.borderRadius = on ? '3px' : '';
      a.style.padding = on ? '0 3px' : '';
    });
  };
